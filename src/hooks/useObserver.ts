...
const useObservation = (setState: Dispatch<SetStateAction<string>>, headingElements: HTMLElement[]) => {
  ...
  const handleIntersect: IntersectionObserverCallback = useCallback((entries: IntersectionObserverEntry[]) => {
    headingElementsRef.current = {}

    // 헤더 태그의 id를 순회하여 headingElementRef에 키 밸류 형태로 할당.
    headingElementsRef.current = entries.reduce((map: ObservationType, headingElement) => {
  ...
  }, [headingElements, handleIntersect])
}
